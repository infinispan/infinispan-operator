...
pipeline {
    ...
    post {
        failure {
            ...
            sh 'kubectl config get-contexts -o name | grep -q kind-xsite1 && kubectl logs daemonset/speaker -n metallb-system --context kind-xsite1'
            sh 'kubectl config get-contexts -o name | grep -q kind-xsite1 && kubectl logs deployment/controller -n metallb-system --context kind-xsite1'
            sh 'kubectl config get-contexts -o name | grep -q kind-xsite2 && kubectl logs daemonset/speaker -n metallb-system --context kind-xsite2'
            sh 'kubectl config get-contexts -o name | grep -q kind-xsite2 && kubectl logs deployment/controller -n metallb-system --context kind-xsite2'
            sh 'df -h'
            sh 'docker ps -a'
