...
4. Optionally install the Infinispan operator on OKD from the bundle file.
  ...
  * If your project name is `infinispan-operator`, run this command to install the operator:
  ```
  $ oc apply -f https://raw.githubusercontent.com/infinispan/infinispan-operator/2.1.x/deploy/operator-install.yaml
  ```
5. Open a new terminal window and create an Infinispan cluster with two nodes:
```
$ oc apply -f https://raw.githubusercontent.com/infinispan/infinispan-operator/2.1.x/deploy/cr/minimal/cr_minimal.yaml
...
4. Optionally install the Infinispan operator on Kubernetes from the bundle file.
  ...
  * If your namespace name is `infinispan-operator`, run this command to install the operator:
  ```
  $ kubectl apply -f https://raw.githubusercontent.com/infinispan/infinispan-operator/2.1.x/deploy/operator-install.yaml
  ```
5. Open a new terminal window and create an Infinispan cluster with two nodes:
```bash
$ kubectl apply -f https://raw.githubusercontent.com/infinispan/infinispan-operator/2.1.x/deploy/cr/minimal/cr_minimal.yaml
```
6. Watch the pods until they are running
```bash
$ kubectl get pods -w
...
Infinispan resources are defined in [infinispan-types.go](https://github.com/infinispan/infinispan-operator/blob/2.1.x/pkg/apis/infinispan/v1/infinispan_types.go).
...
Check the
[official operator documentation](https://infinispan.org/docs/infinispan-operator/2.1.x/operator.html)
for more details.
