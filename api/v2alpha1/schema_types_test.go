...
func TestGetSchemaName(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Uses spec.Name when set and does not double-append .proto",
			schema: Schema{
		...
		{
			name: "Falls back to ObjectMeta.Name and appends .proto suffix",
			schema: Schema{
...
func TestSchemaDefault(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Appends .proto suffix to spec.Name",
			schema: Schema{
				Spec: SchemaSpec{Name: "person"},
			},
		...
		{
			name: "Does not modify empty spec.Name",
			schema: Schema{
				Spec: SchemaSpec{},
			},
			expectedName: "",
...
func TestSchemaSetCondition(t *testing.T) {
	...
	changed := schema.SetCondition(ConditionReady, metav1.ConditionTrue, "")
	assert.True(t, changed)
	assert.Len(t, schema.Status.Conditions, 1)
	assert.Equal(t, ConditionReady, schema.Status.Conditions[0].Type)
	...
	changed = schema.SetCondition(ConditionReady, metav1.ConditionFalse, "some error")
	assert.True(t, changed)
	assert.Len(t, schema.Status.Conditions, 1)
	...
	changed = schema.SetCondition(ConditionReady, metav1.ConditionFalse, "different error")
	assert.True(t, changed)
...
func TestSchemaGetCondition(t *testing.T) {
	...
	cond := schema.GetCondition(ConditionReady)
	assert.Equal(t, ConditionReady, cond.Type)
	assert.Equal(t, metav1.ConditionFalse, cond.Status)

	// Returns existing condition
	schema.SetCondition(ConditionReady, metav1.ConditionTrue, "all good")
	cond = schema.GetCondition(ConditionReady)
