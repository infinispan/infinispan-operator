...
var _ = Describe("Schema Webhook", func() {
	...
	Context("Schema", func() {
		...
		It("Should reject duplicate schema for same cluster and schema name", func() {
			first := &Schema{
				ObjectMeta: metav1.ObjectMeta{
					Name:      key.Name,
					Namespace: key.Namespace,
			...
			expectInvalidErrStatus(err,
				statusDetailCause{metav1.CauseTypeFieldValueDuplicate, "spec.name", "Schema CR already exists for cluster 'some-cluster'"},
			)
		})
