		...
		for _, schemaRef := range instance.Spec.SchemaRefs {
			...
			if err := r.Get(ctx, types.NamespacedName{Namespace: instance.Namespace, Name: schemaRef.Name}, schemaInstance); err != nil {
				if errors.IsNotFound(err) {
					...
					return ctrl.Result{Requeue: true}, cache.update(func() error {
						instance.SetCondition(v2alpha1.CacheConditionReady, metav1.ConditionFalse, fmt.Sprintf("Schema %s not found", schemaRef.Name))
						return nil
					})
				}
				return ctrl.Result{}, err
			...
			if schemaInstance.GetCondition(v2alpha1.SchemaConditionReady).Status != metav1.ConditionTrue {
				...
				return ctrl.Result{Requeue: true}, cache.update(func() error {
					instance.SetCondition(v2alpha1.CacheConditionReady, metav1.ConditionFalse, fmt.Sprintf("Schema %s not ready", schemaRef.Name))
					return nil
				})
			}
		}
