	...
	builder.Watches(
		...
		handler.EnqueueRequestsFromMapFunc(
			func(watchCtx context.Context, a client.Object) []reconcile.Request {
				...
				if s.GetCondition(v2alpha1.ConditionReady).Status != metav1.ConditionTrue {
					return nil
				}

				var requests []reconcile.Request
				cacheList := &v2alpha1.CacheList{}
				if err := r.List(watchCtx, cacheList, &client.ListOptions{Namespace: a.GetNamespace()}); err != nil {
					r.log.Error(err, "watches failed to list Cache CRs")
					return nil
				}

				for _, item := range cacheList.Items {
					if item.HasSchemaRef(a.GetName()) {
						requests = append(requests, reconcile.Request{NamespacedName: types.NamespacedName{Namespace: item.GetNamespace(), Name: item.GetName()}})
					}
				}
				return requests
...
func (r *CacheReconciler) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	...
	if len(instance.Spec.SchemaRefs) > 0 && !crDeleted {
		for _, schemaRef := range instance.Spec.SchemaRefs {
			...
			if err := r.Get(ctx, types.NamespacedName{Namespace: instance.Namespace, Name: schemaRef.Name}, schemaInstance); err != nil {
				if errors.IsNotFound(err) {
					reqLogger.Info(fmt.Sprintf("Schema %s not found, requeuing", schemaRef.Name))
					return ctrl.Result{Requeue: true}, cache.update(func() error {
						instance.SetCondition(v2alpha1.ConditionReady, metav1.ConditionFalse, fmt.Sprintf("Schema %s not found", schemaRef.Name))
						return nil
					})
				}
				return ctrl.Result{}, err
			}
			if schemaInstance.Spec.ClusterName != instance.Spec.ClusterName {
				msg := fmt.Sprintf("Schema %s targets cluster %s, not %s", schemaRef.Name, schemaInstance.Spec.ClusterName, instance.Spec.ClusterName)
				reqLogger.Info(msg)
				return ctrl.Result{}, cache.update(func() error {
			...
			if schemaInstance.GetCondition(v2alpha1.ConditionReady).Status != metav1.ConditionTrue {
				reqLogger.Info(fmt.Sprintf("Schema %s not ready, requeuing", schemaRef.Name))
				return ctrl.Result{Requeue: true}, cache.update(func() error {
					instance.SetCondition(v2alpha1.ConditionReady, metav1.ConditionFalse, fmt.Sprintf("Schema %s not ready", schemaRef.Name))
					return nil
				})
			}
		}
