	...
	builder.Watches(
		...
		handler.EnqueueRequestsFromMapFunc(
			func(_ context.Context, a client.Object) []reconcile.Request {
				cache := a.(*v2alpha1.Cache)
				var requests []reconcile.Request
				for _, ref := range cache.Spec.SchemaRefs {
					requests = append(requests, reconcile.Request{NamespacedName: types.NamespacedName{Namespace: cache.Namespace, Name: ref.Name}})
				}
				return requests
			}),
	)
...
func (r *SchemaReconciler) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	...
	// Bidirectional sync requires operand >= 16.1 and ConfigListener enabled
	operand, err := r.versionManager.WithRef(infinispan.Spec.Version)
	if err != nil {
		return ctrl.Result{}, fmt.Errorf("unable to resolve operand version: %w", err)
	}
	bidirectionalSync := operand.UpstreamVersion.GTE(constants.MinVersionSchemaReconciliation) && infinispan.IsConfigListenerEnabled()
	if !bidirectionalSync {
		var msg string
		if !operand.UpstreamVersion.GTE(constants.MinVersionSchemaReconciliation) {
			msg = fmt.Sprintf("Bidirectional sync requires Infinispan >= %s, current version is %s. Schema changes on the server will not be detected.",
				constants.MinVersionSchemaReconciliation, operand.UpstreamVersion)
		} else {
	...
	if crDeleted {
		if controllerutil.ContainsFinalizer(instance, constants.InfinispanFinalizer) {
			// Check if any Cache CRs reference this Schema
			referencingCaches, err := s.findReferencingCaches()
			if err != nil {
				return ctrl.Result{}, err
			}
	...
	err = s.update(func() error {
		...
		if bidirectionalSync {
			instance.SetCondition(v2alpha1.ConditionBidirectionalSync, metav1.ConditionTrue, "")
		}
		if !controllerutil.ContainsFinalizer(instance, constants.InfinispanFinalizer) {
			controllerutil.AddFinalizer(instance, constants.InfinispanFinalizer)
		}
...
func (r *schemaRequest) reconcileOnServer() bool {
	if val, exists := r.schema.Annotations[constants.ListenerAnnotationGeneration]; exists {
...
func (r *schemaRequest) markedForDeletion() bool {
	_, exists := r.schema.Annotations[constants.ListenerAnnotationDelete]
	return exists
}

func (r *schemaRequest) findReferencingCaches() ([]string, error) {
	cacheList := &v2alpha1.CacheList{}
	if err := r.List(r.ctx, cacheList, &client.ListOptions{Namespace: r.schema.Namespace}); err != nil {
		return nil, fmt.Errorf("unable to list Cache CRs: %w", err)
	}
	var names []string
	for _, c := range cacheList.Items {
		if c.Spec.ClusterName == r.schema.Spec.ClusterName && c.HasSchemaRef(r.schema.Name) {
...
func (sl *SchemaListener) CreateOrUpdate(data []byte) error {
	...
	schemaName, proto, schemaErrors, err := unmarshallSchemaEventConfig(data)
	if err != nil {
		return err
	}
	...
	if existingSchema == nil {
		...
		for i := 1; i <= maxRetries; i++ {
			...
			_, err = controllerutil.CreateOrPatch(sl.Ctx, k8sClient, existingSchema, func() error {
				...
				if existingSchema.Annotations == nil {
					existingSchema.Annotations = make(map[string]string, 1)
				}
...
func (sl *SchemaListener) Delete(data []byte) error {
	...
	if existingSchemaCr.Annotations != nil {
		_, controllerDelete := existingSchemaCr.Annotations[constants.ListenerControllerDelete]
		if controllerDelete {
			...
			return updateSchema(existingSchemaCr, sl.Ctx, sl.Kubernetes.Client, func() error {
				delete(existingSchemaCr.Annotations, constants.ListenerControllerDelete)
				return nil
			})
		}
	}
	...
	if errors.IsNotFound(err) {
		sl.Log.Debugf("Schema CR '%s' not found, nothing to do.", s.Name)
		return nil
	}
	return err
}
...
// unmarshallSchemaEventConfig extracts the schema name, proto content, and any errors from SSE event data
// Event format: {"schema":{"name":"person.proto","errors":"","proto":"message Person { ... }"}}
func unmarshallSchemaEventConfig(data []byte) (name, proto, schemaErrors string, err error) {
	...
	return event.Schema.Name, event.Schema.Proto, event.Schema.Errors, nil
}
