...
func (r *SchemaReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) (err error) {
	...
	builder.Watches(
		&source.Kind{Type: &v2alpha1.Cache{}},
		handler.EnqueueRequestsFromMapFunc(
			func(a client.Object) []reconcile.Request {
...
func (r *SchemaReconciler) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	...
	if !bidirectionalSync {
		...
		if err := s.update(func() error {
			instance.SetCondition(v2alpha1.SchemaConditionBidirectionalSync, metav1.ConditionFalse, msg)
			return nil
		}); err != nil {
			return ctrl.Result{}, err
		}
	...
	if crDeleted {
		if controllerutil.ContainsFinalizer(instance, constants.InfinispanFinalizer) {
			...
			if len(referencingCaches) > 0 {
				...
				reqLogger.Info(msg)
				return ctrl.Result{}, s.update(func() error {
					instance.SetCondition(v2alpha1.SchemaConditionReady, metav1.ConditionFalse, msg)
					return nil
				})
			}
	...
	err = s.update(func() error {
		...
		if bidirectionalSync {
			instance.SetCondition(v2alpha1.SchemaConditionBidirectionalSync, metav1.ConditionTrue, "")
		}
		if !controllerutil.ContainsFinalizer(instance, constants.InfinispanFinalizer) {
...
func (r *schemaRequest) findReferencingCaches() ([]string, error) {
	...
	for _, c := range cacheList.Items {
		if c.HasSchemaRef(r.schema.Name) {
...
func (sl *SchemaListener) CreateOrUpdate(data []byte) error {
	...
	if existingSchema == nil {
		// Create new Schema CR with a deterministic name derived from the cluster and schema names
		sanitizedName := schemaNameRegexp.ReplaceAllString(strcase.ToKebab(schemaName), "-")
		resourceName := clusterName + "-" + sanitizedName
		errs := validation.IsDNS1123Subdomain(resourceName)
		if len(errs) > 0 {
