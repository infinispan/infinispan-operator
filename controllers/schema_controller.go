...
func (r *SchemaReconciler) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	...
	// Schema reconciliation requires operand >= 16.1
	...
	if operand.UpstreamVersion.LT(constants.MinVersionSchemaReconciliation) {
		msg := fmt.Sprintf("Schema CRs require Infinispan >= %s, current version is %s", constants.MinVersionSchemaReconciliation, operand.UpstreamVersion)
		reqLogger.Info(msg)
		return ctrl.Result{}, s.update(func() error {
