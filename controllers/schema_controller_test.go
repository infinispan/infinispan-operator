...
func TestUnmarshallSchemaEventConfig(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedErrors: "Syntax error at line 3",
		},
		{
			name:      "Invalid JSON",
			data:      `not json`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			name, proto, schemaErrors, err := unmarshallSchemaEventConfig([]byte(tt.data))
			if tt.expectErr {
				assert.Error(t, err)
			} else {
				...
				assert.Equal(t, tt.expectedErrors, schemaErrors)
			}
		})
	}
}
