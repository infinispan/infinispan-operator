...
[IMPORTANT]
====
The `spec.service.container.storage`, `spec.service.container.ephemeralStorage`, and `spec.service.container.storageClassName` fields are immutable.
You cannot change these values after you create your {brandname} cluster.
To use different storage configuration you must delete the existing cluster and create a new one.
====
