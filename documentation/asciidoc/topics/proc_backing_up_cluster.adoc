...
* Decide which {brandname} resources you want to back up.
...
Client write operations might not be archived if they occur after the cache is backed up.
To archive the exact state of the cluster, you should ensure there are no active client connections before you back it up.
====
...
. Check {brandname} logs for the following message:
+
[source,options="nowrap",subs=attributes+]
----
...
----
+
. Run the following command to verify that the backup is successfully created:
+
[source,options="nowrap",subs=attributes+]
----
$ {describe_backup}
