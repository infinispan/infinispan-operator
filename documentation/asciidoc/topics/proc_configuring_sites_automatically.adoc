...
+
You should configure all pods in the cluster to send RELAY messages for better performance.
This allows all pods to send replication requests to backup locations directly, rather than forwarding the requests to relay pods.
+
