...
ifdef::community[]
* Set up {openshiftshort} or {k8s} for cross-site replication.
endif::community[]
ifdef::downstream[]
* Set up {openshift} for cross-site replication.
endif::downstream[]
...
+
You should configure all nodes in the cluster to send RELAY messages for better performance.
