...
= Specifying {brandname} Cluster static hosts and ports for Cross-Site Replication
Specify static hosts and ports for {brandname} clusters so they can establish connections and form cross-site views.

.Prerequisites

* Ensure that you have the correct hosts and port numbers for each {brandname} cluster that you plan to configure as a backup location.

.Procedure

. Create an `Infinispan` CR for each {brandname} cluster.
. Specify the name of the local site with `spec.service.sites.local.name`.
. Provide the name and static host and port URL for each {brandname} cluster that acts as a backup location with `spec.service.sites.locations`.
. Port can be omitted and default `7900` value will be used.
+
...
spec:
  ...
  service:
    ...
    sites:
      ...
      locations:
        - name: LON
          url: xsite://infinispan-lon.myhost.com:7900
        - name: NYC
          url: xsite://infinispan-nyc.myhost.com:7900
----
+
* **NYC**
+
    ...
    sites:
      ...
      locations:
        - name: NYC
          url: xsite://infinispan-nyc.myhost.com:7900
        - name: LON
          url: xsite://infinispan-lon.myhost.com
----
+
