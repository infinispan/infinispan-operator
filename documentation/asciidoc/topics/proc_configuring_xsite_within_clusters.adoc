...
. Provide the hostname in the URL for each {brandname} cluster that acts as a backup location with `spec.service.sites.locations`.
+
...
spec:
  ...
  service:
    ...
    sites:
      ...
      locations:
        - name: SiteA
          url: xsite://example-clustera-site <1>
        - name: SiteB
          url: xsite://example-clusterb-site
----
<1> The value of the hostname in the `URL` field is the {k8s} service name that resolves to an internal IP address.
+
