...
. Optionally, if your cache uses `application/x-protostream` encoding, specify `Schema` CR dependencies with the `spec.schemaRefs` field.
When set, {ispn_operator} waits until all referenced `Schema` CRs are ready before creating the cache.
See xref:schemas_{context}[] for more information.
