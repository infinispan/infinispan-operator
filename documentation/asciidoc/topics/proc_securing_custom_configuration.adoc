...
. Apply the changes.
. Open your {brandname} Server for editing.
. Add a `credential-reference` in the `security`  section of the configuration.
. Specify `credentials` as the name of the `store`.
. Specify the key of the secret with the `alias` attribute.
+
.{brandname}.xml
----
