...
{ispn_operator} adds a resource type and custom resource definition (CRD)
...
<2> sets the resource type that ispn_operator} uses to handle the lifecycle of {brandname} clusters.
