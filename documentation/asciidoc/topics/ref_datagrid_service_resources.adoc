...
|`spec.configListener.logging.levels`
|Configures the logging level used by the ConfigListener deployments. Can be one of `debug | info | error`. The default level is `info`.
