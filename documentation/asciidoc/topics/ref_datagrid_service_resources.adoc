...
|`spec.configListener`
|Creates a `listener` pod in each {brandname} cluster that allows {ispn_operator} to reconcile server-side modifications with `Infinispan` resources such as the `Cache` CR.
...
You should do this only if you access `Infinispan` resources through the {brandname} Console, CLI, or client applications.
