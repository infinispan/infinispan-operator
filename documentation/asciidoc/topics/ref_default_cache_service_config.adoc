...
[role="_abstract"]
This topic describes default cache configuration for {cacheservice} nodes.
...
* Define the maximum size for the data container in bytes. {ispn_operator} calculates the maximum size when it creates nodes.
* Evict cache entries to control the size of the data container. You can enable automatic scaling so that {ispn_operator} adds nodes when memory usage increases instead of removing entries.
