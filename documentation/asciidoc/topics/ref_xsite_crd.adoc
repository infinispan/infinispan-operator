...
<8> Directly specifies the `host` routing to another {brandname} backup instance.
<9> Directly specifies the backup instance JGroups `port` the relay is listening on.
