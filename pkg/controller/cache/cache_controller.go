...
func (r *ReconcileCache) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	for i, c := range ispnInstance.Status.Conditions {
		if c.Type == "wellFormed" && c.Status == "True" {
			break
		}
		if i == len(ispnInstance.Status.Conditions)-1 {
	...
	labelSelector := labels.SelectorFromSet(util.GetLabelsForInfinispan(ispnInstance.Name, ""))
	...
	labelSelector = labels.SelectorFromSet(ispnutil.GetLabelsForInfinispan(ispnInstance.Name, "infinispan-service"))
	...
	if len(serviceList.Items) != 1 {
		err = fmt.Errorf("Zero or more than 1 service associated to the cluster")
		reqLogger.Error(err, fmt.Sprintf("Size of the result list: %d", len(serviceList.Items)))
		return reconcile.Result{}, err
	}
	...
	statusUpdate = statusUpdate || setCondition(instance, "Ready", "True", "")
	if statusUpdate {
...
func setCondition(cache *v2alpha1.Cache, condition string, status metav1.ConditionStatus, message string) bool {
	changed := false
	...
	cache.Status.Conditions = append(cache.Status.Conditions, v2alpha1.CacheCondition{Type: condition, Status: status, Message: message})
	return true
}
