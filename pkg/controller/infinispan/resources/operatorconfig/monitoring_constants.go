		...
		"templating": {
		  "list": [
			...
			},
			{
			  ...
			  "datasource": "Prometheus",
			  "definition": "query_result({job=~\"$cluster\", namespace=~\"$namespace\", __name__=~\"vendor_cache_manager_default_cache.*_activation_activation_count\"})",
			  "hide": 0,
			  "includeAll": true,
			  "label": null,
			  "multi": true,
			  "name": "caches",
			  "options": [],
			  "query": "query_result({job=~\"$cluster\", namespace=~\"$namespace\", __name__=~\"vendor_cache_manager_default_cache.*_activation_activation_count\"})",
			  "refresh": 1,
			  "regex": "/vendor_cache_manager_default_cache_(.*)_activation_activation_count/",
			  "skipUrlSync": false,
			  "sort": 0,
			  "tagValuesQuery": "",
			  "tags": [],
