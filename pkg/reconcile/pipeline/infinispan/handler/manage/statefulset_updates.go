...
func StatefulSetRollingUpgrade(i *ispnv1.Infinispan, ctx pipeline.Context) {
	...
	if requested := i.TerminationGracePeriodSeconds(); requested != nil && !reflect.DeepEqual(spec.TerminationGracePeriodSeconds, requested) {
		spec.TerminationGracePeriodSeconds = requested
		updateNeeded = true
