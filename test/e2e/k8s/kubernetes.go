...
func (k TestKubernetes) WaitForExternalService(routeName string, timeout time.Duration, client *http.Client, user, password, namespace string) string {
	...
	err := wait.Poll(tconst.DefaultPollPeriod, timeout, func() (done bool, err error) {
		...
		if strings.Contains(hostAndPort, "127.0.0.1") {
			...
			hostAndPort = fmt.Sprintf("127.0.0.1:%d", tconst.DefaultHotRodPort)
			result := checkExternalAddress(client, user, password, hostAndPort)
			if result {
				return result, nil
			}
...
func (k TestKubernetes) getExternalAddress(route *v1.Service) (string, error) {
	...
	if len(route.Status.LoadBalancer.Ingress) == 1 {
		if route.Status.LoadBalancer.Ingress[0].IP != "" {
			return fmt.Sprintf(route.Status.LoadBalancer.Ingress[0].IP+":%d", tconst.DefaultHotRodPort), nil
		}
		if route.Status.LoadBalancer.Ingress[0].Hostname != "" {
			return fmt.Sprintf(route.Status.LoadBalancer.Ingress[0].Hostname+":%d", tconst.DefaultHotRodPort), nil
		}
	}
...
func (k TestKubernetes) installRBAC(namespace string) {
	...
	yamlReader, err := testutils.GetYamlReaderFromFile("../../deploy/role.yaml")
	utils.ExpectNoError(err)
	...
	yamlReader, err = testutils.GetYamlReaderFromFile("../../deploy/service_account.yaml")
	utils.ExpectNoError(err)
	...
	yamlReader, err = testutils.GetYamlReaderFromFile("../../deploy/role_binding.yaml")
	utils.ExpectNoError(err)
