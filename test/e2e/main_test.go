...
var cluster = ispncluster.NewCluster(kubernetes.Kubernetes)

var DefaultSpec = ispnv1.Infinispan{
	TypeMeta: tconstants.InfinispanTypeMeta,
	ObjectMeta: metav1.ObjectMeta{
		Name: tconstants.DefaultClusterName,
	},
	Spec: ispnv1.InfinispanSpec{
		...
		Container: ispnv1.InfinispanContainerSpec{
			CPU:    tconstants.CPU,
			Memory: tconstants.Memory,
		},
		Image:    tconstants.ImageName,
		Replicas: 1,
...
func TestMain(m *testing.M) {
	if tconstants.RunLocalOperator == "true" {
...
func TestNodeStartup(t *testing.T) {
	...
	// Register it
	kubernetes.CreateInfinispan(spec, tconstants.Namespace)
	defer kubernetes.DeleteInfinispan(spec, tconstants.SinglePodTimeout)
...
func TestClusterFormationWithTLS(t *testing.T) {
	...
	// Create secret
	kubernetes.CreateSecret(&encryptionSecret, tconstants.Namespace)
...
func genericTestForContainerUpdated(ispn ispnv1.Infinispan, modifier func(*ispnv1.Infinispan), verifier func(*appsv1.StatefulSet)) {
	...
	spec.TypeMeta = tconstants.InfinispanTypeMeta
	...
	err = wait.Poll(tconstants.DefaultPollPeriod, tconstants.SinglePodTimeout, func() (done bool, err error) {
...
func TestCacheService(t *testing.T) {
	...
	user := "developer"
	password, err := cluster.Kubernetes.GetPassword(user, spec.GetSecretName(), tconstants.Namespace)
	testutil.ExpectNoError(err)

	routeName := fmt.Sprintf("%s-external", name)
	client := &http.Client{}
	hostAddr := kubernetes.WaitForExternalService(routeName, tconstants.RouteTimeout, client, user, password, tconstants.Namespace)
...
func TestPermanentCache(t *testing.T) {
	...
	var createPermanentCache = func(ispn *ispnv1.Infinispan) {
		pass, err := cluster.Kubernetes.GetPassword(usr, ispn.GetSecretName(), tconstants.Namespace)
		testutil.ExpectNoError(err)
		routeName := fmt.Sprintf("%s-external", name)
		client := &http.Client{}
		hostAddr := kubernetes.WaitForExternalService(routeName, tconstants.RouteTimeout, client, usr, pass, tconstants.Namespace)
...
func genericTestForGracefulShutdown(clusterName string, modifier func(*ispnv1.Infinispan), verifier func(*ispnv1.Infinispan)) {
	...
	// Delete the cluster
	kubernetes.GracefulShutdownInfinispan(spec, tconstants.SinglePodTimeout)
	kubernetes.GracefulRestartInfinispan(spec, 1, tconstants.SinglePodTimeout)
...
func waitForPodsOrFail(spec *ispnv1.Infinispan, num int) {
	...
	kubernetes.WaitForPods("app=infinispan-pod", num, tconstants.SinglePodTimeout, tconstants.Namespace)

	pods := kubernetes.GetPods("app=infinispan-pod", tconstants.Namespace)
	podName := pods[0].Name
	...
	var lastErr error
	err := wait.Poll(time.Second, tconstants.TestTimeout, func() (done bool, err error) {
		value, err := cluster.GetClusterSize(secretName, podName, tconstants.Namespace, protocol)
		if err != nil {
			lastErr = err
			return false, nil
		}
...
func TestExternalService(t *testing.T) {
	...
	kubernetes.WaitForPods("app=infinispan-pod", 1, tconstants.SinglePodTimeout, tconstants.Namespace)

	pass, err := cluster.Kubernetes.GetPassword(usr, spec.GetSecretName(), tconstants.Namespace)
	testutil.ExpectNoError(err)
...
func exposeServiceType() corev1.ServiceType {
	switch tconstants.ExposeServiceType {
	case "ClusterIP":
	...
	default:
		panic(fmt.Errorf("unknown service type %s", tconstants.ExposeServiceType))
	}
}
...
func TestExternalServiceWithAuth(t *testing.T) {
	...
	identities := security.CreateIdentitiesFor(usr, pass)
	...
	kubernetes.CreateSecret(&secret, tconstants.Namespace)
	...
	identities = security.CreateIdentitiesFor(usr, newpass)
	identitiesYaml, err = yaml.Marshal(identities)
	testutil.ExpectNoError(err)
	...
	kubernetes.CreateSecret(&secret1, tconstants.Namespace)
