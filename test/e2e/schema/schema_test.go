...
func skipIfSchemaNotSupported(t *testing.T) {
	if tutils.CurrentOperand.UpstreamVersion.LT(cconsts.MinVersionSchemaReconciliation) {
		t.Skipf("Schema CRs require Infinispan >= %s, current operand is %s", cconsts.MinVersionSchemaReconciliation, tutils.CurrentOperand.UpstreamVersion)
	}
}
...
`
	...
	cache.Spec.SchemaRefs = []string{schemaCRName}
...
`
	...
	cacheConfig := `{"distributed-cache":{"encoding":{"media-type":"application/x-protostream"},"indexing":{"indexed-entities":["book_sample.Book","author_sample.Author"]}}}`
	cache := cacheCR(cacheName, cacheConfig, ispn)
	cache.Spec.SchemaRefs = []string{bookCRName, authorCRName}
	testKube.Create(cache)
