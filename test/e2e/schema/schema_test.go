...
func TestSchemaClusterRecreate(t *testing.T) {
	...
	testKube.WaitForSchemaCondition(schemaName, ispn.Name, tutils.Namespace, v2alpha1.Condition{
...
func TestSchemaInUseProtection(t *testing.T) {
	...
	testKube.WaitForSchemaState(schemaName, ispn.Name, tutils.Namespace, func(s *v2alpha1.Schema) bool {
		...
		c := s.GetCondition(v2alpha1.ConditionReady)
...
// TestSchemaWithInvalidProto verifies that a Schema CR with invalid protobuf content
// is marked as not ready with an error message.
func TestSchemaWithInvalidProto(t *testing.T) {
	t.Parallel()
	defer testKube.CleanNamespaceAndLogOnPanic(t, tutils.Namespace)

	ispn := initCluster(t, false)
	schemaName := ispn.Name + "-invalid.proto"

	invalidProto := `this is not valid protobuf`

	schema := schemaCR(schemaName, invalidProto, ispn)
	testKube.Create(schema)

	// Schema should become not-ready with an error message
	testKube.WaitForSchemaState(schemaName, ispn.Name, tutils.Namespace, func(s *v2alpha1.Schema) bool {
		c := s.GetCondition(v2alpha1.ConditionReady)
		return c.Status == metav1.ConditionFalse && c.Message != ""
	})

	// Fix the schema with valid content
	schema = testKube.FindSchemaResource(schemaName, ispn.Name, tutils.Namespace)
	schema.Spec.Schema = sampleProto
	testKube.Update(schema)
...
// TestSchemaDuplicateMessage verifies that registering two schemas with the same
// package and message type causes the second schema to be marked as not ready.
func TestSchemaDuplicateMessage(t *testing.T) {
	t.Parallel()
	defer testKube.CleanNamespaceAndLogOnPanic(t, tutils.Namespace)
...
`
	schema1Name := ispn.Name + "-dup1.proto"
	schema1 := schemaCR(schema1Name, proto, ispn)
	testKube.Create(schema1)
	testKube.WaitForSchemaConditionReady(schema1Name, ispn.Name, tutils.Namespace)

	// Create a second schema with the same package and message
	schema2Name := ispn.Name + "-dup2.proto"
	schema2 := schemaCR(schema2Name, proto, ispn)
	testKube.Create(schema2)

	// The second schema (or both) should be marked as not ready due to duplicate types
	testKube.WaitForSchemaState(schema2Name, ispn.Name, tutils.Namespace, func(s *v2alpha1.Schema) bool {
		c := s.GetCondition(v2alpha1.ConditionReady)
		return c.Status == metav1.ConditionFalse && c.Message != ""
	})

	// Clean up: delete schema2, then verify schema1 becomes ready again
	testKube.DeleteSchema(schema2)
	testKube.WaitForSchemaConditionReady(schema1Name, ispn.Name, tutils.Namespace)

	testKube.DeleteSchema(schema1)
}
